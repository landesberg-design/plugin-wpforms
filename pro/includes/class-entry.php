...
class WPForms_Entry_Handler extends WPForms_DB {
	...
	/**
	 * List of an editable fields type.
	 *
	 * @since 1.7.0
	 *
	 * @return array
	 */
	public function get_editable_field_types(): array {
	...
	public function get_column_defaults() {
		...
		return [
			...
			'date_modified' => date( 'Y-m-d H:i:s' ), // phpcs:ignore WordPress.DateTime.RestrictedFunctions.date_date
			'ip_address'    => '',
			'user_agent'    => '',
			'user_uuid'     => '',
		];
	 ...
	 * @noinspection PhpParameterNameChangedDuringInheritanceInspection
	 */
	public function delete( $entry_id = 0, $args = [] ): bool {
	...
	private function insert_legacy_payment( $entry, $payment_meta ) {  // phpcs:ignore Generic.Metrics.CyclomaticComplexity.MaxExceeded
		...
		$payment_meta = [
			...
			'payment_recipient'   => $payment_meta['payment_recipient'] ?? '',
			'receipt_number'      => $payment_meta['receipt_number'] ?? '',
